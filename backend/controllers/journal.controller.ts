...
import { createJournalEntry, fetchJournalEntries, fetchJournalEntryById } from '../services/journals.service';
...
const createJournal = async (req: Request, res: Response) => {
	try {
		const { user_id, ambience, text }: { user_id: string; ambience: string; text: string } = req.body;

		if (!user_id || !text || !ambience) {
			return res.status(400).json({ error: 'Missing fields' });
		}

		const result = await createJournalEntry({ user_id, text, ambience });
...
const getAnalysis = async (req: Request, res: Response) => {
	try {
		...
		const { emotion, summary, keywords } = await analyzeJournalEntry({ journal_id });
		return res.status(200).json({ emotion, summary, keywords });
	} catch (err: any) {
		console.error(err);
		return res.status(500).json({ error: 'Internal server error' });
	}
